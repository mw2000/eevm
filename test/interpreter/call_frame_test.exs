defmodule EEVM.CallFrameTest do
  ...
  test "push_frame stores parent frame and switches execution context" do
    ...
    assert state_after_push.frame.code == child_frame.code
    assert state_after_push.frame.contract.address == 2
  ...
  test "pop_frame restores parent and writes return data into parent memory" do
    ...
    child_finished =
      MachineState.update_frame(%{state_after_push | status: :stopped}, fn frame ->
        %{frame | return_data: <<0xAA, 0xBB>>}
      end)

    {:ok, resumed_state} = MachineState.pop_frame(child_finished)
    {bytes, _} = Memory.read_bytes(resumed_state.frame.memory, 4, 4)

    assert resumed_state.frame.contract.address == 1
    assert resumed_state.status == :running
    assert resumed_state.frame.gas == 950
  ...
  test "run_loop pops halted child frame and resumes parent" do
    ...
    assert result.frame.contract.address == 10
