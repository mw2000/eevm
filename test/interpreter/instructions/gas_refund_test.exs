defmodule EEVM.Interpreter.Instructions.GasRefundTest do
  ...
  alias EEVM.Interpreter.MachineState.Frame
  ...
  test "add_refund increments refund counter" do
    ...
    assert updated.frame.refund == 15
  end

  test "sub_refund decrements with floor at 0" do
    state = MachineState.new(<<0x00>>, refund: 10)

    assert MachineState.sub_refund(state, 3).frame.refund == 7
    assert MachineState.sub_refund(state, 100).frame.refund == 0
  end
  ...
  test "Prague floors final charged gas after refunds to calldata floor cost" do
    ...
    assert prague_result.frame.refund == 0
    assert tx.gas_limit - prague_result.frame.gas == 21_010

    assert cancun_result.status == :stopped
    assert tx.gas_limit - cancun_result.frame.gas == 21_008
  end
  ...
  test "nested call revert restores parent refund snapshot" do
    ...
    {:ok, state_after_push} = MachineState.push_frame(parent_state, child_frame)
    child_reverted = %{state_after_push | status: :reverted}

    result = Interpreter.run_loop(child_reverted)

    assert result.status == :stopped
    assert result.call_stack == []
    assert result.frame.refund == 7
  end
end
